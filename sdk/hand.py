...
class DexterousHand:
    ...
    def __init__(self, arm_ip: str, com_port: int = 1, roh_addr: int = 2, delay: float = 1.0):
        ...
        # 初始化机械臂通讯
        self.robot = RobotArmController(arm_ip, 8080, 3)  # type: ignore
    ...
    def open_all(self):
        ...
        self._write_registers(ROH_FINGER_POS_TARGET0, [0, 0, 0, 0, 0])  # type: ignore
        time.sleep(self.delay)

    def close_all(self):
        """闭合所有手指"""
        print("[ACTION] Close all fingers")
        self._write_registers(ROH_FINGER_POS_TARGET0, [0, 65535, 65535, 65535, 65535])  # type: ignore
        # time.sleep(self.delay)
